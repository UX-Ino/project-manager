                ...
                (() => {
                  ...
                  if (a11yViewMode === 'dashboard') {
                    ...
                    a11yItems.forEach(item => {
                      ...
                      // 위반 사항인 경우 (checked === false)
                      if (!item.checked) {
                        totalViolations++;

                        // 심각도 계산
                        const lowerError = errorMsg.toLowerCase();
                        if (lowerError.includes('치명적') || lowerError.includes('critical') || lowerError.includes('(critical)') || lowerError.includes('상') || lowerError.includes('[상]')) {
                          criticalViolations++;
                        } else if (lowerError.includes('중요') || lowerError.includes('serious') || lowerError.includes('(serious)') || lowerError.includes('중') || lowerError.includes('[중]')) {
                          seriousViolations++;
                        }
                    ...
                    return (
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div className="bg-white p-5 rounded-2xl border border-[#e5e8eb] shadow-sm flex flex-col justify-between h-[120px] transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
                            <span className="text-xs font-semibold text-[#8b95a1]">총 위반 건수</span>
                            <span className="text-3xl font-extrabold text-[#f04452]">{totalViolations} <span className="text-sm font-bold text-[#f04452]/70">건</span></span>
                          </div>
                          ...
                          <div className="bg-[#fdf3f4] p-5 rounded-2xl border border-[#fce8e6] shadow-sm flex flex-col justify-between h-[120px] transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
                            <span className="text-xs font-semibold text-[#c5221f]">치명적 (Critical)</span>
                            <span className="text-3xl font-extrabold text-[#c5221f]">{criticalViolations} <span className="text-sm font-bold text-[#c5221f]/70">건</span></span>
                          </div>
                          ...
                          <div className="bg-[#fff9eb] p-5 rounded-2xl border border-[#fef7e0] shadow-sm flex flex-col justify-between h-[120px] transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
                            <span className="text-xs font-semibold text-[#b06000]">중요 (Serious)</span>
                            <span className="text-3xl font-extrabold text-[#b06000]">{seriousViolations} <span className="text-sm font-bold text-[#b06000]/70">건</span></span>
                          </div>
                        </div>
                        ...
                        {/* 원칙별 위반 현황 카드 */}
                        <div className="bg-white p-6 rounded-2xl border border-[#e5e8eb] shadow-sm space-y-5 transition-all duration-200 hover:shadow-md">
                          <div>
                            <h3 className="text-sm font-bold text-[#191f28]">원칙별 위반 현황</h3>
                            <p className="text-[11px] text-[#8b95a1] mt-0.5">지침 범주에 따른 위반 건수 상세 분포</p>
                          </div>

                          <div className="space-y-4">
                            {[
                              { label: '기타', count: others, fromColor: '#9ca3af', toColor: '#6b7280', emoji: '⚙️' },
                              { label: '운용의 용이성', count: operation, fromColor: '#34d399', toColor: '#059669', emoji: '⌨️' },
                              { label: '견고성', count: robustness, fromColor: '#c084fc', toColor: '#8b5cf6', emoji: '🏗️' },
                              { label: '인식의 용이성', count: recognition, fromColor: '#60a5fa', toColor: '#3182f6', emoji: '👁️' },
                              { label: '이해의 용이성', count: understanding, fromColor: '#fcd34d', toColor: '#f59e0b', emoji: '🧠' }
                            ]
                              ...
                              return (
                                <div key={idx} className="space-y-1.5">
                                  <div className="flex justify-between items-center text-xs">
                                    <span className="font-semibold text-[#4e5968] flex items-center gap-1">
                                      <span>{item.emoji}</span>
                                      <span>{item.label}</span>
                                    </span>
                                    <span className="font-bold text-[#191f28]">{item.count} 건</span>
                                  </div>
                              ...
                              return (
                                <tr key={item.id} style={{ backgroundColor: rowBg, borderBottom: cellBorder }}>
                                  ...
                                  <td className="py-2 px-3 text-center text-xs" style={{ borderRight: cellBorder }}>
                                    {item.tag ? (
                                      <span className={`inline-block text-[10px] px-1.5 py-0.5 rounded font-semibold ${
                                        item.tag.includes('완료') || item.tag.includes('배포') ? 'bg-[#e6f4ea] text-[#137333] border border-[#137333]/10' :
                                        item.tag.includes('대기') || item.tag.includes('진행') ? 'bg-[#e8f0fe] text-[#1a73e8] border border-[#1a73e8]/10' :
                                        'bg-[#f1f3f5] text-[#5f6368] border border-[#5f6368]/10'
                                      }`}>
                                        {item.tag}
                                      </span>
                                    ) : '—'}
                                  ...
                                  <td className="py-2 px-3 text-center text-xs" style={{ borderRight: cellBorder }}>
                                    {checkStatus ? (
                                      <span className={`inline-block text-[10px] px-1.5 py-0.5 rounded font-semibold ${
                                        checkStatus.includes('적합') || checkStatus.includes('완료') || checkStatus.includes('통과') || checkStatus.includes('OK') ? 'bg-[#e6f4ea] text-[#137333] border border-[#137333]/10' :
                                        checkStatus.includes('오류') || checkStatus.includes('부적합') || checkStatus.includes('미흡') || checkStatus.includes('NG') ? 'bg-[#fce8e6] text-[#c5221f] border border-[#c5221f]/10' :
