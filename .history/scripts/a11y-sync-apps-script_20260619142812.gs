...
function _cloneTemplate() {
  ...
  var newFile  = DriveApp.getFileById(CONFIG.TEMPLATE_SLIDE_ID).makeCopy(fileName);
...
function _showCompletionDialog(ui, pres, taskCount) {
  var html = HtmlService.createHtmlOutput(
    ...
    '  <p>총 <b>' + taskCount + '</b>개의 작업 내역이 변환되었습니다.</p>' +
    '  <a href="' + pres.getUrl() + '" target="_blank"' +
...
function _extractUrlFromCell(sheet, rowNum) {
  try {
    ...
    _logWarning("extractUrlFromCell", "행 " + rowNum + " 이미지 추출 실패: " + e.message);
    return "";
  }
}
...
function _updateCoverSlide(pres, todayStr) {
  pres.getSlides()[CONFIG.SLIDE.COVER]
      .replaceAllText(CONFIG.TAG.DATE, todayStr);
}
...
function _updateListSlides(pres, tasks) {
  ...
  var slides       = _duplicateSlide(pres, listTemplate, CONFIG.SLIDE.LIST, chunks.length);
...
function _fillListTable(slide, chunk) {
  ...
  chunk.forEach(function(task, rowIdx) {
    ...
    var cells = [
      table.getCell(r, 0), table.getCell(r, 1), table.getCell(r, 2),
      table.getCell(r, 3), table.getCell(r, 4),
    ];
...
function _updateDetailSlides(pres, tasks, listPageCount) {
  ...
  tasks.forEach(function(task) {
    var newSlide = pres.insertSlide(pres.getSlides().length - 1, detailTemplate);
...
function _replaceSlideImage(slide, url) {
  ...
  try {
    ...
    if (code !== 200) {
      _logWarning("replaceSlideImage", "이미지 로드 실패 (HTTP " + code + "): " + url);
      return;
    }

    var newImg = slide.insertImage(response.getBlob());
    var ratio  = newImg.getHeight() / newImg.getWidth();
    newImg.setLeft(left).setTop(top).setWidth(width).setHeight(width * ratio);

  } catch (e) {
    _logWarning("replaceSlideImage", "이미지 삽입 실패: " + url + " → " + e.message);
  }
}

function _getLargestImage(images) {
  return images.reduce(function(prev, curr) {
    return (prev.getWidth() * prev.getHeight() >= curr.getWidth() * curr.getHeight())
      ? prev : curr;
  });
}
...
var A11Y_CONFIG = {
  ...
  API_ENDPOINT: "https://050ccbebcb1277.lhr.life/api/a11y-sync",
...
function syncA11yToSupabase() {
  ...
  if (rows.length === 0) {
    ...
    if (lastRow >= A11Y_CONFIG.DATA_START_ROW) {
      ...
      var rangeVal = sheet.getRange(A11Y_CONFIG.DATA_START_ROW, 1, numRows, maxCols).getValues();
      for (var k = 0; k < rangeVal.length; k++) {
        sampleRows.push("행 " + (A11Y_CONFIG.DATA_START_ROW + k) + ": [" + rangeVal[k].join(" | ") + "]");
      }
    }
    var msg = "동기화할 유효한 데이터가 없습니다.\n\n" +
              "현재 로드된 시트: \"" + sheet.getName() + "\"\n" +
              "데이터 시작행(DATA_START_ROW): " + A11Y_CONFIG.DATA_START_ROW + "\n" +
              "시트의 마지막 행 번호: " + lastRow + "행\n\n" +
              "처음 " + sampleRows.length + "개 행 데이터 샘플:\n" +
              sampleRows.join("\n") + "\n\n" +
...
function _a11yCallSyncApi(rows) {
  ...
  try {
    ...
    try {
      ...
      return {
        success: false,
        error: "[HTTP " + code + "] 서버가 JSON을 반환하지 않았습니다: " + contentText,
      };
    }
