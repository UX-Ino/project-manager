/**
 * ============================================================
 *  WBS 구글 시트 → Next.js API → Supabase DB 동기화 스크립트
 *  (보안 강화를 위해 Next.js API Route를 거쳐 동기화 수행)
 * ============================================================
 *
 * 📋 설치 방법:
 *  1. 구글 시트 열기
 ...
 *                   (로컬 테스트 시 ngrok 터널링 주소 + /api/wbs-sync 사용)
...
/**
 * WBS 시트 데이터를 읽어 Next.js API를 통해 Supabase DB에 동기화합니다.
...
function syncWbsToSupabase() {
  ...
  // 3. WBS 데이터 읽기
  Logger.log("[WBS Sync] 시트 데이터 읽기 시작...");
  ...
  if (result.success) {
    ...
    showAlert("동기화 성공", result.message);
  } else {
...
function validateConfig() {
  if (CONFIG.API_ENDPOINT.startsWith("https://your-")) {
    showAlert(
      "설정 오류",
      "CONFIG.API_ENDPOINT를 실제 API 엔드포인트 URL로 변경해 주세요."
    );
    return false;
  }
...
function readWbsRows(sheet) {
  ...
  var range = sheet.getRange(CONFIG.DATA_START_ROW, 1, numRows, 16);
...
function onOpen() {
  try {
    ...
    ui.createMenu("🔄 WBS 동기화")
      .addItem("DB로 동기화 실행", "syncWbsToSupabase")
      .addToUi();
  } catch (e) {
